st_discretize_multilines_1 = function(network_object, section_length_threshold, input_crs, segmentation_length){
  ...
  message("Making cooridor metric tables")
  ...
  message("Perfroming first reordering")
  ...
  temp_igraph_reorded = temp_igraph %>%
    ...
    dplyr::group_map(~{
      ...
      if (!check_inf){
        ...
        message("ALERT!!!!!! Not and actual fail error.")
  ...
  temp_segemented_links = temp_igraph_reorded %>%
    ...
    dplyr::group_map(~{
      ...
      tryCatch({
        ...
        if (temp_shape_length > (segmentation_length+2) #& nrow(temp_sf_object) != 
        ){
          #need to skip if length less than 10
          temp_points = gauntletMap::st_line_sample_to_points_pro(
  ...
  message("Perfroming second reordering process")
  ...
  temp_segemented_links_seg_ordered = temp_segemented_links %>%
    ...
    dplyr::group_map(~{
      ...
      if (!check_inf){
        ...
        temp_pro = x %>%
          dplyr::mutate(cor_seg_ordr = 1) %>%
          dplyr::mutate(cor_seg_ordr_pro = 1
                        ,cor_seg_ordr_pro_pct = 1)
      }
  ...
  message(str_glue("{gauntlet::strg_make_space_2()}Ended Run Successfully!\n++++++++++++++++++++++++++++++++++++++++++++++++++"))
